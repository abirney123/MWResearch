...
linear model weights and test set features. Each plot has three lines, one for raw
...
def train_models(X_train, y_train, X_test, models, PCA_flag, random_state, threshold):
    """
    ...
    results : Dictionary
        Nested dictionary of training results. First level keys are model names 
        as specified in the models dictionary. Second level keys are "y_scores"
        and hold the y_scores for on the test set for the model denoted by the
        first level key.
    X_test : DataFrame
...
def plot_auroc_f1(aurocs, f1s, window_size):
    ...
    fig.suptitle(f"All Models Test AUROC and F1: {window_size}s Sliding Window")
    ...
    plt.savefig(f"./onset_v_self_report/OnTest/noCV/{window_size}s/test_auroc_f1_onset_self_report_{window_size}s_win")
    plt.show()
...
def plot_probs(test_results, X_test_relative_times, true_labels_test_set, window_size):
    ...
    for model_name, results in test_results.items():
        ...
        plt.tight_layout()
        plt.savefig(f"./onset_v_self_report/OnTest/noCV/{window_size}s/{model_name}_probabilities")
        plt.show()
        
def plot_raw_scores(X_test_relative_times, true_labels_test_set, window_size, raw_scores):
    """
    ...
    for model_name, results in raw_scores.items():
        ...
        plt.tight_layout()
        plt.savefig(f"./onset_v_self_report/OnTest/noCV/{window_size}s/{model_name}_raw_scores")
        plt.show()
...
def plot_rf_feat_importance(importances, columns, window_size, classifier_type):
    ...
    plt.ylabel('Weight')
    plt.tight_layout()
    plt.savefig(f"./onset_v_self_report/OnTest/noCV/{window_size}s/rf_feat_importance_{classifier_type}")
    plt.show()

def get_raw_scores(mw_models, sr_models, mw_2_models, X_test_mw, X_test_mw2, X_test_sr):
    """
    ...
    for model_name, model in mw_models.items():
        ...
        if model_name in linear_models:
            ...
            if hasattr(model, "coef_"):
                ...
                Z = np.dot(X_test_mw, w)  # array, one entry for each sample: these are raw scores
                # add to dict
                results[model_name]["MW_onset_raw_scores"] = pd.Series(Z, index=X_test.index) 
            else:
    ...
    for model_name, model in sr_models.items():
        ...
        if model_name in linear_models:
            ...
            if hasattr(model, "coef_"):
                w = model.coef_.flatten() # list, one entry for each feature
                # find Z - dot prod of X_test and weights
                Z = np.dot(X_test_sr, w)  # array, one entry for each sample: these are raw scores
                # add to dict
                results[model_name]["self_report_raw_scores"] = pd.Series(Z, index = X_test.index) 
            else:
                print("The specified model doesn't have the coef_ attribute...")
    ...
    for model_name, model in mw_2_models.items():
        ...
        if model_name in linear_models:
            ...
            if hasattr(model, "coef_"):
                w = model.coef_.flatten() # list, one entry for each feature
                # find Z - dot prod of X_test and weights
                Z = np.dot(X_test_mw2, w)  # array, one entry for each sample: these are raw scores
                # add to dict
                results[model_name]["MW_onset_2_raw_scores"] = pd.Series(Z, index=X_test.index) 
            else:
                print("The specified model doesn't have the coef_ attribute...")
                
    return results

def plot_forward_feature_importance(raw_scores, columns, window_size, X_test_mw, X_test_mw2, X_test_sr):
    """
    ...
    for model_name, scores in raw_scores.items():
        ...
        A_numerator_mw = np.dot(X_test_mw.T, Z_mw) # transpose X so inner dimensions match for mult.
        ...
        A_numerator_mw2 = np.dot(X_test_mw2.T, Z_mw2) # transpose X so inner dimensions match for mult.
        ...
        A_numerator_sr = np.dot(X_test_sr.T, Z_sr) # transpose X so inner dimensions match for mult.
        ...
        plt.savefig(f"onset_v_self_report/OnTest/noCV/{window_size}s/{model_name}_forward_feat_importance.png") 
        plt.show()
        
def predictor_hist(raw_scores, y_test, classifier_type, window_size):
    """
    ...
    plt.tight_layout()  
    plt.savefig(f"onset_v_self_report/OnTest/noCV/{window_size}s/Predictor_hist_{classifier_type}.png")
...
self_report_models, self_report_results, X_test_sr = train_models(X_train_self_report,
                                                           y_train_self_report,
                                                           X_test, self_report_models,
                                                           PCA_flag, random_state, PCA_threshold)
...
MW_onset_models, MW_onset_results, X_test_mw = train_models(X_train_MW_onset,
                                                           y_train_MW_onset,
                                                           X_test, MW_onset_models,
                                                           PCA_flag, random_state, PCA_threshold)
...
MW_onset_2_models, MW_onset_2_results, X_test_mw2 = train_models(X_train_MW_onset_2,
                                                           y_train_MW_onset_2,
                                                           X_test, MW_onset_2_models,
                                                           PCA_flag, random_state, PCA_threshold)

# turn test sets back into data frames if turned into arrays through pca
if PCA_flag:
    X_test_mw = pd.DataFrame(X_test_mw)
    X_test_mw2 = pd.DataFrame(X_test_mw2)
    X_test_sr = pd.DataFrame(X_test_sr)
...
raw_scores = get_raw_scores(MW_onset_models, self_report_models, MW_onset_2_models, X_test_mw, X_test_mw2, X_test_sr)
plot_raw_scores(X_test_relative_times, true_labels_test_set, window_size, raw_scores)

if PCA_flag == False:
    plot_forward_feature_importance(raw_scores, columns, window_size, X_test_mw, X_test_mw2, X_test_sr)

# plot predictor hists 

predictor_hist(raw_scores, y_test, "mw_onset", window_size)
predictor_hist(raw_scores, y_test, "self_report", window_size)
predictor_hist(raw_scores, y_test, "mw_onset_2", window_size)
